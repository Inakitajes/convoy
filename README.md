...
Model messages in `session`, phase reports, and run summaries render their Markdown with distinct heading, emphasis, list, quote, link, and code styles. From a phase's `reports` tab, press `v` for a full-screen reader; press `c` there to copy the complete original report, and `v` or `Esc` to return.
