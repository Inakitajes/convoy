...
**Reason**: Feature identity, registered contract sets, association revisions, and ownership/rebinding are removed rather than replaced with another registry.

**Migration**: Review an explicit execution checkout and zero or more ordered local changes in the run plan. Legacy feature records and plan links are inert; missing target evidence requires explicit fresh review, not adoption. Existing no-spec execution and separate dirty-tree consent remain supported.
