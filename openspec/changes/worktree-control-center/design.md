...
Push and cleanup remain optional. Worktree removal retains its branch by default and checks tracked/untracked/ignored content, submodules, locks, active managed writers, and current target identity. In this version, valuable ignored content blocks ordinary removal until dealt with explicitly; no force-removal shortcut. Never remove main or the current process checkout; show deferred guarded commands when needed.
...
- **Ignored files/submodules may hold valuable content** → Conservative explicit removal checks and no force shortcut; leave cleanup blocked rather than erase unknown content.
